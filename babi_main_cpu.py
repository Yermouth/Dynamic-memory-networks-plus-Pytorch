...
def position_encoding(embedded_sentence, use_cuda=False):
    '''
...
class InputModule(nn.Module):
    ...
    def forward(self, contexts, word_embedding, use_cuda=False):
        '''
        ...
        h0 = Variable(cuda(torch.zeros(2, batch_num, self.hidden_size), use_cuda))
...
class DMNPlus(nn.Module):
    def __init__(self, hidden_size, vocab_size, num_hop=3, qa=None, use_cuda=False):
        super(DMNPlus, self).__init__()
        self.num_hop = num_hop
        self.qa = qa
        self.word_embedding = cuda(nn.Embedding(vocab_size, hidden_size, padding_idx=0, sparse=True), use_cuda)
